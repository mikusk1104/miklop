...
def parseInterface(message, lastTime):
  ...
  dropsErrors = int(lineRXdrops) + int(lineTXdrops) + int(lineRXerrors) + int(lineTXerrors)
  parsedLine = lineHostName + '_INTERFACES' + ',Interface=' + lineIntName + ' RX=' + lineRXbits + ',TX=' + lineTXbits + ',DropsAndErrors=' + str(dropsErrors) + ' ' + str(int(lineDate.timestamp()))
